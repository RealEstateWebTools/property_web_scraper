...
module PropertyWebScraper
  RSpec.describe "MlsImportMapper" do
    ...
    #   mapped_property = MlsImportMapper.new("mls_csv_jon").map_property(property_tsv)
    #   expect(mapped_property).to include("reference" => property_tsv["ML Number"])
    # end

    it "maps olr data correctly" do
      mapped_property = MlsImportMapper.new("mls_olr").map_property(property_olr)
    ...
    it "maps mris data correctly" do
      mapped_property = MlsImportMapper.new("mls_mris").map_property(property_mris)
      expect(mapped_property).to include("reference" => property_mris["ListingKey"])
    end

    it "maps interealty data correctly" do
      mapped_property = MlsImportMapper.new("mls_interealty").map_property(property_interealty)
