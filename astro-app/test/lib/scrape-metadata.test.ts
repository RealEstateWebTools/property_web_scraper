...
import type { QualityGrade } from '../../src/lib/extractor/quality-scorer.js';
    ...
    it('includes diagnostics fields when provided', async () => {
      const scrape = await recordScrapeAndUpdatePortal(makeInput({
        diagnostics: {
          ...
          qualityGrade: 'B' as QualityGrade,
          qualityLabel: 'Good',
          successClassification: 'good',
          expectedExtractionRate: 0.7,
          expectedQualityGrade: 'B' as QualityGrade,
          populatedFields: 15,
          totalFields: 30,
    ...
    it('archives old profile when signature changes', async () => {
      ...
      await recordScrapeAndUpdatePortal(makeInput({
        ...
        diagnostics: {
          ...
          extractableFields: 20,
          qualityGrade: 'F' as QualityGrade,
          qualityLabel: 'Poor',
          populatedFields: 1,
          totalFields: 30,
          meetsExpectation: false,
    ...
    it('starts at 0 when meets_expectation is true', async () => {
      await recordScrapeAndUpdatePortal(makeInput({
        diagnostics: {
          ...
          expectedQualityGrade: 'A' as QualityGrade,
          populatedFields: 18,
          totalFields: 30,
          meetsExpectation: true,
          expectationGap: 0.05,
