...
test.describe('/api/v1/listings API', () => {
  ...
  test('GET with unsupported URL returns success: false', async ({ request }) => {
    const res = await request.get('/api/v1/listings?url=https://www.unsupported-xyz.com/listing/1');
    const json = await res.json();
    expect(json.success).toBe(false);
  });

  test('GET with supported URL returns success: true with listing', async ({ request }) => {
    const res = await request.get('/api/v1/listings?url=https://www.idealista.com/inmueble/12345/');
    const json = await res.json();
...
test.describe('/api/v1/listings/:id API', () => {
  test('GET with invalid ID returns 404', async ({ request }) => {
    const res = await request.get('/api/v1/listings/nonexistent-id');
